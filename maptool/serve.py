...
#   GET  /maptool/editor.html , /v0.4.p8 , ...   (static files)
#   POST /save                                   (writes body -> v0.4.p8)
# Serves from the cortex_override/ dir so the editor can auto-load
# v0.4.p8 and write it back directly (no download step).
...
class H(http.server.SimpleHTTPRequestHandler):
    def do_POST(self):
        if self.path == '/save':
            ...
            open('v0.4.p8', 'wb').write(self.rfile.read(n))
...
print('editor server on http://localhost:8765  (Export writes v0.4.p8)')
