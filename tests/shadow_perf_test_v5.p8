...
function find_near(tr,tg,tb)
  ...
  for i=0,15 do
    if i~=14 then
      local d=abs(pr[i+1]-tr)+abs(pg[i+1]-tg)+abs(pb[i+1]-tb)
...
function _init()
  ...
  -- precompute colored palettes per light
  for i=1,#lights do
    local lt=lights[i]
    lt.pals=make_pals(lt.lr,lt.lg,lt.lb)
  end
...
function draw_multi()
  ...
  -- color overlay: tint the lit screen
  -- copy lit screen to spritesheet so
  -- tint reads brightness-adjusted pixels
  memcpy(0x0000,0x6000,0x2000)
  for li=2,nl do
    local d=ld[li]
    local p=d.pals[4]
